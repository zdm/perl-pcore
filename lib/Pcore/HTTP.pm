...
use Pcore::Util::Scalar qw[blessed is_globref is_ioref];
...
sub request ( @ ) {
    ...
    $args{on_finish} = sub {
        ...
        # rewind body fh
        $res->body->seek( 0, 0 ) if $res->has_body && ( is_globref $res->body || is_ioref $res->body );
...
## |------+----------------------+----------------------------------------------------------------------------------------------------------------|
## |    3 | 161                  | Subroutines::ProhibitExcessComplexity - Subroutine "request" with high complexity score (32)                   |
## |------+----------------------+----------------------------------------------------------------------------------------------------------------|
