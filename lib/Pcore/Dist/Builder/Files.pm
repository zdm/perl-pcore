package Pcore::Dist::Builder::Files;

use Pcore qw[-class];
use Pcore::Dist::Builder::Files::File;
...
around new => sub ( $orig, $self, $source_path ) {
    ...
    P->file->find(
        {   wanted => sub {
                ...
                $files->{ $path->to_string } = Pcore::Dist::Builder::Files::File->new( { path => $path->to_string, source_path => $source_path . $path } );
            },
            no_chdir => 1,
        },
        q[.]
...
sub add_file ( $self, $path, $content_ref ) {
    $self->files->{$path} = Pcore::Dist::Builder::Files::File->new( { path => $path, content => $content_ref } );
