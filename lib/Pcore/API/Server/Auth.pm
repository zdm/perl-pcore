...
has auth => ( is => 'ro', isa => PositiveInt, required => 1 );
...
sub create_user ( $self, $username, $password, $cb ) {
    state $q1 = $self->dbh->query('INSERT OR IGNORE INTO user (username, password, enabled) VALUES (?, 1, 0)');

    state $q2 = $self->dbh->query('UPDATE user SET password = ?, enabled = 1 WHERE id = ?');
...
sub create_token ( $self, $uid, $perms, $cb ) {
    state $q1 = $self->dbh->query('INSERT OR IGNORE INTO token (id, token, uid, enabled) VALUES (?, ?, ?, 1)');
