...
use Pcore -const,
  -export => {
    ...
    QUERY => [qw[SET VALUES WHERE IN GROUP_BY ORDER_BY]],
  };
...
sub get_query ( $self, $dbh, $final, $i ) {
            ...
            for my $field ( keys $token->%* ) {
                ...
                # ArrayRef value is processed as parameter
                    ...
                    if ( $token->{$field}->@* == 1 ) {
                        ...
                        push @bind, $token->{$field}->[0];
                    }
                    else {

                        # validate operator
                        my $operator = $SQL_COMPARISON_OPERATOR->{ lc $token->{$field}->[0] } or die qq[SQL opertaor "$token->{$field}->[0]" is not allowed];

                        push @buf, $quoted_field . ' ' . $operator . ' $' . $i->$*++;

                        push @bind, $token->{$field}->[1];
                    }
                }
                else {
                    die 'Unsupported ref type';
    ...
    if (@sql) {
        return ( $final ? 'WHERE ' : q[] ) . join( q[ ], @sql ), \@bind;
    }
    else {
        return;
    }
...
## | Sev. | Lines                | Policy                                                                                                         |
## |======+======================+================================================================================================================|
## |    3 | 413                  | Subroutines::ProhibitExcessComplexity - Subroutine "get_query" with high complexity score (23)                 |
## |------+----------------------+----------------------------------------------------------------------------------------------------------------|
## |    2 | 475, 603             | ValuesAndExpressions::ProhibitEmptyQuotes - Quotes used with a string containing no non-whitespace characters  |
## +------+----------------------+----------------------------------------------------------------------------------------------------------------+
##
