package Pcore::App::API::Backend::Local::pgsql::AppInstance;
...
sub get_app_instance ( $self, $app_instance_id, $cb ) {
    $self->dbh->selectrow(
        q[SELECT * FROM "api_app_instance" WHERE "id" = ?],
        [$app_instance_id],
        sub ( $dbh, $res, $app_instance ) {
            if ($app_instance) {
                ...
                $self->_generate_token(
                    ...
                    sub ( $token ) {
                        ...
                        # app instance token generated
                            ...
                            $self->dbh->do(
                                q[INSERT INTO "api_app_instance" ("id", "app_id", "version", "host", "created_ts", "hash") VALUES (?, ?, ?, ?, ?, ?)],
                                [ $token->{data}->{id}, $app->{data}->{id}, $app_instance_version, $app_instance_host, time, $token->{data}->{hash} ],
                                sub ( $dbh, $res, $data ) {
                                    if ( !$res ) {
    ...
    $self->_generate_app_instance_token(
        ...
        sub ( $token ) {
            ...
            # app instance token generated
                ...
                $self->dbh->do(
                    q[UPDATE "api_app_instance" SET "hash" = ? WHERE "id" = ?],
                    [ $token->{data}->{hash}, $app_instance_id ],
                    sub ( $dbh, $res, $data ) {
                        if ($res) {
...
sub update_app_instance ( $self, $app_instance_id, $app_instance_version, $cb ) {
    $self->dbh->do(
        q[UPDATE "api_app_instance" SET "version" = ?, "last_connected_ts" = ? WHERE "id" = ?],
        [ $app_instance_version, time, $app_instance_id ],
        sub ( $dbh, $res, $data ) {
            if ( !$res ) {
...
## | Sev. | Lines                | Policy                                                                                                         |
## |======+======================+================================================================================================================|
## |    3 | 9, 123, 223          | Subroutines::ProhibitManyArgs - Too many arguments                                                             |
## |------+----------------------+----------------------------------------------------------------------------------------------------------------|
...
Pcore::App::API::Backend::Local::pgsql::AppInstance
